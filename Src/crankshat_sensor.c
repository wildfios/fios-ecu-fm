...
int16_t meanPulsWidth = 0;
uint8_t syncCout = 0; // TODO: check this funcking thing
uint8_t toothInterval = 0;
uint8_t oldToothInterval = 0;
...
void set_multipled_frq(uint16_t meanPulsWidth) {
	__HAL_TIM_SET_AUTORELOAD(&htim3,
				(uint16_t)meanPulsWidth / FRQ_MUL_FACTOR); /* Multimplied period for tooth counter */
		__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1,
				(uint16_t)meanPulsWidth / (FRQ_MUL_FACTOR * 2)); /* Multiplied pulse PWM output. Testing prepose only */
		__HAL_TIM_SET_COUNTER(&htim3, 0); /* Null the counter */
}

void process_crank_period(uint32_t period) {
	uint8_t curToothCnt = toothInterval;

	if (meanPulsWidth < period * 2) {
		angle = 0;
	} else {
...
//	/* caululating meanPulsWidth */
//	if (meanPulsWidth == 0) {
//		/* Initial meanPulsWidth value, cold start */
//		meanPulsWidth = period;
//	}
//
//	if (meanPulsWidth * 2 > period * 1.1) {  //mesure_puse_diff(period, meanPulsWidth) == 1) {
//		set_multipled_frq(period);
//		meanPulsWidth = period;
//		rpmVal = period; /* Storing RPM value for dashboard */
//	} else {
//		angle = 0;
//	}
}
...
void process_crank_pulse(TIM_HandleTypeDef *htim) {
	...
	if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
		...
		if (period == 0) set_multipled_frq(period);
		process_crank_period(period);
	}
}
 ...
 *
 * */
//void process_crank_period_old(uint32_t period) {
...
//			set_multipled_frq(meanPulsWidth);
//			crankError = 0; /*  */
